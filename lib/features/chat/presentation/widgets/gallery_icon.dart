...
class GalleryIcon extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Icon(Icons.attach_file_outlined, color: ChatColors.actionIconsColor, size: 25.w),
    ).splash(onPressed: () async {
      await sendFiles(context);
    });
  }
  ...
  Future<void> sendFiles(BuildContext context) async {
    try {
      ...
      final pickedFiles = result?.files.where((e) => validString(e.path)).toList();
