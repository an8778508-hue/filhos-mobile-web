...
import 'package:escola/features/add_form/add_form_type.dart';
...
class MedicinesProfessorsRepo {
  final NetworkClientRepository networkClient;

  MedicinesProfessorsRepo({required this.networkClient});
  ...
  Future<Either<Failure, List<MedicineModel>>> getMedicineRequests([int page = 1]) async {
    final List<Either<Failure, dynamic>> f = await Future.wait([
      di<AddFormRepo>(instanceName: AddFormType.medicine.name).fetchFields(),
      networkClient.handleRequest(
          NetworkRequest(
            method: HttpMethod.get,
            url: 'teacher/medicines/requests',
            queryParameters: {'page': '$page'},
          ),
          onSuccess: (json) => jsonDecode(jsonEncode(json))),
    ]);
          ...
          if (validList(medicines)) {
            ...
            for (int medicineIndex = 0; medicineIndex < medicines.length; medicineIndex++) {
              ...
              if (validMap(payload)) {
                ...
                medicinesJson['data'][dataIndex]['medicines'][medicineIndex]['raw'] = {...payload};
  ...
  Future<Either<Failure, List<MedicinePrescriptionModel>>> getMedicineHistory([int page = 1]) async {
    return networkClient.handleRequest(
        NetworkRequest(
          method: HttpMethod.get,
          url: '/teacher/medicines/children/medicines-history',
          queryParameters: {'page': '$page'},
        ),
        onSuccess: (json) => validateDataList(json['data'], (json) {
              return MedicinePrescriptionModel(
                child: ChildModel.fromJson(json['child']),
                feed: validateDataList(json['medicines'], PrescriptionModel.fromJson),
              );
            }));
  }

  Future<Either<Failure, List<MedicinePrescriptionModel>>> getMedicineReminder([int page = 1]) async {
    return networkClient.handleRequest(
        NetworkRequest(
          method: HttpMethod.get,
          url: '/teacher/medicines/reminders',
          queryParameters: {'page': '$page'},
        ),
        onSuccess: (json) => validateDataList(json['data'], MedicinePrescriptionModel.fromJson));
  }

  Future<Either<Failure, void>> markMedicineReminder(String id) async {
    return networkClient.handleRequest(
        NetworkRequest(
          method: HttpMethod.put,
          url: '/teacher/medicines/reminders/$id',
          body: {
            "read":1,
            "reminder_id":id,
          }
  ...
  Future<Either<Failure, void>> acceptRequest(
    ...
    return networkClient.handleRequest(
        NetworkRequest(
          ...
          url: 'teacher/medicines/requests/$id',
          body: {
            'medicine_id':id,
            'status': APPROVED,
          }
  ...
  Future<Either<Failure, void>> rejectRequest(String id, String reason, List<File> attachments) async {
    return networkClient.handleRequest(
        NetworkRequest(
          method: HttpMethod.post,
          url: 'teacher/medicines/requests/$id',
...
extension RepoExtension on MedicinesProfessorsRepo {
